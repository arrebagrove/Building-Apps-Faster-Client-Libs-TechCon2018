...
namespace Demo
{
    class Program
    {
        ...
        private static async Task RunAsync()
        {
            ...
            string baseUrl = "https://marc-rras.osisoft.int/piwebapi";
            ...
            JObject dataServer = await MakeRequest(baseUrl + "/dataservers?name=MARC-PI2016", "GET");
            JObject sinusoidPoint = await MakeRequest(baseUrl + "/points?path=\\\\marc-pi2016\\sinusoid", "GET");
            JObject element = await MakeRequest(baseUrl + "/elements?path=\\\\MARC-PI2016\\CrossPlatformLab\\marc.adm", "GET");
            JObject attribute = await MakeRequest(baseUrl + "/attributes?path=\\\\MARC-PI2016\\CrossPlatformLab\\marc.adm|Heading", "GET");
            ...
            try
            {
                JObject homeLanding2 = await MakeRequest("https://devdata.osisoft.com/piwebapi/", "GET");
            }
            catch (Exception e)
            {
            ...
            try
            {
                JObject point = await MakeRequest(baseUrl + "/points?path=\\\\marc-pi2016\\sinusoid3424", "GET");
            }
            catch (Exception e)
            {
                Console.WriteLine("Expected error: {0}", e.Message);
            ...
            JObject point1 = await MakeRequest(baseUrl + "/points?path=\\\\marc-pi2016\\sinusoid", "GET");
            JObject point2 = await MakeRequest(baseUrl + "/points?path=\\\\marc-pi2016\\sinusoidu", "GET");
            JObject point3 = await MakeRequest(baseUrl + "/points?path=\\\\marc-pi2016\\cdt158", "GET");
            ...
            batch["3"] = new JObject();
            batch["1"].Method = "GET";
            batch["1"].Resource = "https://marc-rras.osisoft.int/piwebapi/points?path=\\\\marc-pi2016\\sinusoid";
            batch["2"].Method = "GET";
            batch["2"].Resource = "https://marc-rras.osisoft.int/piwebapi/points?path=\\\\marc-pi2016\\cdt158";
            batch["3"].Method = "GET";
            batch["3"].Resource = "https://marc-rras.osisoft.int/piwebapi/streamsets/value?webid={0}&webid={1}";
        ...
        private static async Task<JObject> MakeRequest(string url, string method, object data = null)
        {
            ...
            handler.Credentials = new NetworkCredential("marc.adm", "kk");
